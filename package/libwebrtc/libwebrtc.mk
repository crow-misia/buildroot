...
LIBWEBRTC_VERSION = b484ec0082948ae086c2ba4142b4d2bf8bc4dd4b
LIBWEBRTC_SOURCE =
...
LIBWEBRTC_PATH = $(BR_PATH):$(DEPOT_TOOLS)
...
define LIBWEBRTC_EXTRACT_CMDS
	...
	export SSL_CERT_DIR="$(TARGET_DIR)/etc/ssl/certs" && \
	export PATH="$(LIBWEBRTC_PATH)" && \
...
define LIBWEBRTC_INSTALL_STAGING_CMDS
	...
	cd $(@D)/src/third_party/abseil-cpp && \
	for h in $$(find . -type f -name '*.h'); do \
	  $(INSTALL) -m 0644 -D $$h $(STAGING_DIR)/usr/include/webrtc/$$h || exit 1; \
	done
	cd $(@D)/src/third_party/boringssl/src/include && \
	for h in $$(find . -type f -name '*.h'); do \
	  $(INSTALL) -m 0644 -D $$h $(STAGING_DIR)/usr/include/webrtc/$$h || exit 1; \
	done
	cd $(@D)/src/third_party/jsoncpp/source/include && \
	for h in $$(find . -type f -name '*.h'); do \
	  $(INSTALL) -m 0644 -D $$h $(STAGING_DIR)/usr/include/webrtc/$$h || exit 1; \
	done
	cd $(@D)/src/third_party/libyuv/include && \
	for h in $$(find . -type f -name '*.h'); do \
	  $(INSTALL) -m 0644 -D $$h $(STAGING_DIR)/usr/include/webrtc/$$h || exit 1; \
	done
