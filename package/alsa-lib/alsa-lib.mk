...
ALSA_LIB_LICENSE = LGPL-2.1+ (library)
...
ifneq ($(BR2_PACKAGE_ALSA_LIB_ASERVER),y)
define ALSA_LIB_REMOVE_ASERVER
	rm -f $(TARGET_DIR)/usr/bin/aserver
endef
ALSA_LIB_POST_INSTALL_TARGET_HOOKS += ALSA_LIB_REMOVE_ASERVER
else
ALSA_LIB_LICENSE += GPL-2.0+ (aserver)
